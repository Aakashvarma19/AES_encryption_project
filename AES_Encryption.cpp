...
{  
    ...
    while(i < 4*(Nr+1))  
    {  
        word temp = w[i-1]; //Record the previous word  
        if(i % Nk == 0)  
...
{  
    ...
    for(int i=0;i<Nt;i++)
    {
		if(in.read((char*)&d[i],sizeof(d[i])))
		{
			divideToByte(p[i],d[i]);
			threads.push_back(thread(encrypt,p[i],wo[i]));
		}
		else
		{
			flag=false;
   ...
   while(flag)
    ...
    for(int i=0;i<Nt;i++)
    {
		...
		{
			divideToByte(p[i],d[i]);
			threads.push_back(thread(decrypt,p[i],wo[i]));
		}
		else
		{
			flag=false;
